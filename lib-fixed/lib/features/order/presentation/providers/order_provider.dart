...
import 'package:frozen_food_1123150049/features/order/data/repositories/order_repository_impl.dart';
import 'package:frozen_food_1123150049/features/order/domain/repositories/order_repository.dart';
...
class OrderProvider extends ChangeNotifier {
  ...
  /// Konfirmasi ke backend bahwa CashLess sudah melaporkan sukses.
  /// Dipanggil dari PaymentPendingPage saat callback sukses diterima —
  /// tanpa ini status order di backend tidak akan pernah berubah dari
  /// "pending", walau pembayaran di CashLess sudah berhasil.
  Future<bool> confirmPayment(int orderId) async {
    try {
      final updated = await _repository.confirmPayment(orderId);
      _lastOrder = updated;
      _paymentCheckStatus = PaymentCheckStatus.paid;
      notifyListeners();
      return true;
    } catch (e) {
      // Konfirmasi gagal (mis. koneksi putus) — biarkan polling reguler
      // yang menangkap perubahan status berikutnya.
      return false;
    }
  }
