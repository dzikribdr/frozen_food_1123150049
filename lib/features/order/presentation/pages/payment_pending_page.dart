...
class _PaymentPendingPageState extends State<PaymentPendingPage>
  ...
  void _onPaymentSuccess() async {
    _log(' _onPaymentSuccess dipanggil — konfirmasi ke backend & navigasi');
    final orderProv = context.read<OrderProvider>();
    orderProv.stopPaymentPolling();

    // Penting: tanpa panggilan ini, backend tidak pernah tahu bahwa
    // CashLess sudah melaporkan sukses — status order akan tetap
    // "pending" selamanya walau deep link callback sudah diterima.
    final confirmed = await orderProv.confirmPayment(widget.order.id);
    _log(' confirmPayment → $confirmed');
    ...
    Navigator.pushNamedAndRemoveUntil(
      ...
      arguments: orderProv.lastOrder ?? widget.order,
    );
  }
